...
const baseUrl = 'http://wsjkw.sh.gov.cn/';
const siteTitle = '--上海市卫生健康委员会';

const catrgoryMap = {
    xwfb: { title: '新闻发布', suffix: 'xwfb/' }
};
...
async function load(link) {
    ...
    // 提取文章内容
    const description = $('.container').html();
    // 返回解析的结果
    return { description };
}
...
module.exports = async (ctx) => {
    // 默认 正常规定 然后获取列表页面
    const type = ctx.params.type || 'xwfb';
    ...
    const result = await Promise.all(
        ...
        list
            .map(async (item) => {
                ...
                // 获取绝对路径
                const pattern = new RegExp('/(http|https):\/\/([\w.]+\/?)\S*/');
                ...
                const date_txt = $('span.time').text().match(/[1-9]\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])/); // 匹配 yyyy-mm-dd格式时间
